...
# --- Set Paths ---
input_folder <- "G:/ML_Papers_Data/All_Index_Data/Monthly_SM_Mask/"
output_folder <- "G:/ML_Papers_Data/All_Index_Data/Accces Data for 1st paper/SMCI_N/"

# --- Create Output Folder If It Doesn't Exist ---
if (!dir.exists(output_folder)) dir.create(output_folder, recursive = TRUE)

# --- Load Monthly Soil Moisture Files ---
...
# --- Calculate Min & Max Per Pixel ---
cat("🔍 Calculating min and max values...\n")
sm_min <- app(sm_stack, fun = min, na.rm = TRUE)
sm_max <- app(sm_stack, fun = max, na.rm = TRUE)

# --- Scale to SMCI: 0.1 to 1 ---
cat("⚙️ Calculating SMCI scaled from 0.1 to 1.0...\n")
smci_stack <- ((sm_stack - sm_min) / (sm_max - sm_min)) * 0.9 + 0.1
smci_stack <- clamp(smci_stack, lower = 0.1, upper = 1.0)

# --- Set Output File Names ---
dates <- seq(as.Date("2001-01-01"), as.Date("2020-12-01"), by = "month")

# --- Save Monthly SMCI TIFFs ---
...
cat("🎉 All SMCI files (scaled 0.1 to 1) saved successfully.\n")
